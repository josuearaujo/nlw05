...
socket.on("admin_list_all_users", (connections) => {
  ...
  connections.forEach(connection => {
    ...
    if(usersInSupport.find(user => user === connection.socket_id)){
      const btn = document.getElementById(`btnuser${connection.socket_id}`);
      btn.disabled = true;
      btn.style.cursor = "not-allowed";
      btn.style.backgroundColor = "lightgreen";
      btn.innerHTML = "Em atendimento"
    }
  });
});

function call(id) {
  const btn = document.getElementById(`btnuser${id}`);
  usersInSupport.push(id);
  btn.disabled = true;
  btn.style.cursor = "not-allowed";
  btn.style.backgroundColor = "lightgreen";
  btn.innerHTML = "Em atendimento"
